...
describe("FfClient", function () {
    it("Creates correct packet for small encrypted request", function () { return __awaiter(_this, void 0, void 0, function () {
        var client, request, packets, ptr;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    expect(packets[0].length).toEqual(20 /* Header */ +
                    ...
                    expect(packets[0].payload[ptr++]).toEqual(request_1.FfRequestOptionType.ENCRYPTION_MODE);
                    // Option length (int16)
                    expect((packets[0].payload[ptr++] << 8) + (packets[0].payload[ptr++] << 0)).toEqual(1);
                    // Option payload
                    expect(packets[0].payload[ptr++]).toEqual(request_1.FfEncryptionMode.AES_256_GCM);
                    // Encryption IV Option
                    expect(packets[0].payload[ptr++]).toEqual(request_1.FfRequestOptionType.ENCRYPTION_IV);
                    // Option length (int16)
                    expect((packets[0].payload[ptr++] << 8) + (packets[0].payload[ptr++] << 0)).toEqual(12);
                    // Option payload
                    expect(packets[0].payload.slice(ptr, ptr + 12)).not.toEqual(new Uint8Array(12));
                    ptr += 12;
                    // Encryption Tag Option
                    expect(packets[0].payload[ptr++]).toEqual(request_1.FfRequestOptionType.ENCRYPTION_TAG);
                    // Option length (int16)
                    expect((packets[0].payload[ptr++] << 8) + (packets[0].payload[ptr++] << 0)).toEqual(16);
                    // Option payload
                    expect(packets[0].payload.slice(ptr, ptr + 16)).not.toEqual(new Uint8Array(16));
                    ptr += 16;
                    // EOL Option
                    ...
                    expect(packets[0].payload.slice(ptr)).not.toEqual(Uint8Array.from(Buffer.from(request, "utf-8")));
                    return [2 /*return*/];
            }
        });
    }); });
